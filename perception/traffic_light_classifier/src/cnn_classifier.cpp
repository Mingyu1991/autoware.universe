...
bool CNNClassifier::getTrafficSignal(
  const cv::Mat & input_image, autoware_perception_msgs::msg::TrafficLight & traffic_signal)
{
  if (!trt_common_->isInitialized()) {
...
bool CNNClassifier::postProcess(
  std::vector<float> & output_tensor, autoware_perception_msgs::msg::TrafficLight & traffic_signal,
  bool apply_softmax)
{
