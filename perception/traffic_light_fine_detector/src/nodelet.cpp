...
: Node("traffic_light_fine_detector_node", options)
{
  ...
  const std::string detector_type = this->declare_parameter<std::string>("detector_type");
  ...
  if (fs::exists(engine_path)) {
    ...
    //   }
    // }
    // else if(detector_type == "yolox-s"){
    //   net_ptr_.reset(new tensorrt_yolox::TrtYoloX(engine_path, false));
    //   if (max_batch_size != net_ptr_->getMaxBatchSize()) {
    //     RCLCPP_INFO(
    //       this->get_logger(),
    //       "Required max batch size %d does not correspond to Profile max batch size %d. Rebuild "
    ...
    RCLCPP_INFO(get_logger(), "before load yolo onnx model");
    net_ptr_.reset(new tensorrt_yolox::TrtYoloX(onnx_file, "fp32", 2, score_thresh_, 0.7));
    RCLCPP_INFO(get_logger(), "after load yolo onnx model");
    //net_ptr_->save(engine_path);
  }
...
bool TrafficLightFineDetectorNodelet::getTlrIdFromLabel(
