  ...
  const BehaviorPathPlannerParameters & common_parameter, const PullOverParameters & parameter)
{
  ...
  for (double lateral_jerk = minimum_lateral_jerk; lateral_jerk <= maximum_lateral_jerk;
    ...
    candidate_path.partial_paths.push_back(combineReferencePath(
      road_lane_reference_path, shifted_path.path));  // shift pull over path has only one path
    candidate_path.road_path = road_lane_reference_path;
    candidate_path.pull_over_path = shifted_path.path;
  ...
  const lane_departure_checker::LaneDepartureChecker & lane_departure_checker)
{
  ...
  for (const auto & path : paths) {
    // no need to check here, but in pull_over_module.cpp because all total_distances are same.
    // if (!hasEnoughDistance(
    //       total_distance, current_lanes, target_lanes, current_pose, is_in_goal_route_section,
    //       goal_pose)) {
    //   continue;
    // }

    if (lane_departure_checker.checkPathWillLeaveLane(lanes, path.pull_over_path)) {
      continue;
    }
...
bool hasEnoughDistance(
  const double total_distance, const lanelet::ConstLanelets & current_lanes,
  const Pose & current_pose, const bool is_in_goal_route_section, const Pose & goal_pose)
{
  if (total_distance > util::getDistanceToEndOfLane(current_pose, current_lanes)) {
    return false;
  }

  if (
    is_in_goal_route_section &&
    total_distance > util::getSignedDistance(current_pose, goal_pose, current_lanes)) {
    return false;
  }
