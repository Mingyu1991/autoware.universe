...
BoundingBox3D::BoundingBox3D(
  const tf2::Vector3 & position, const tf2::Quaternion & orientation, double length, double width,
  double height)
...
std::vector<tf2::Vector3> BoundingBox3D::getVertices() const
{
  ...
  for (int x = -1; x <= 1; x += 2) {
    for (int y = -1; y <= 1; y += 2) {
      for (int z = -1; z <= 1; z += 2) {
        vertices[idx++] = transformToWorldFrame(
          tf2::Vector3(0.5 * x * getLength(), 0.5 * y * getWidth(), 0.5 * z * getHeight()));
      }
    }
  }
  return vertices;
...
{
  return std::abs(pt.x()) <= getLength() * 0.5 && std::abs(pt.y()) <= getWidth() * 0.5 &&
